...
import $utils from './lib/cypress/utils';
import $errUtils from './lib/cypress/error_utils';
import $actionability from './lib/cy/actionability';
import { ElViewportPostion } from './lib/dom/coordinates';
...
type MouseActionOptions = {
  ...
  onReady: (fromElViewport: ElViewportPostion, forceEl: any) => any;
  onTable: Function;
...
const mouseAction = (eventName, { subject, positionOrX, y, userOptions, onReady, onTable, defaultOptions }: MouseActionOptions) => {
  ...
  const options = _.defaults({}, userOptions, {
    ...
    y,
    errorOnSelect: true,
    waitForAnimations: cy.config('waitForAnimations'),
    animationDistanceThreshold: cy.config('animationDistanceThreshold'),
    ctrlKey: false,
    controlKey: false,
    altKey: false,
    optionKey: false,
  ...
  const perform = (el, index) => {
    ...
    return $actionability
      .verify(cy, $el, cy.config, individualOptions, {
...
Cypress.Commands.addAll(
  { prevSubject: 'element' },
  {
    mousedown(subject, positionOrX, y, options = {}) {
      return mouseAction('mousedown', {
        ...
        onReady(fromElViewport, forceEl) {
          const clickEvents = mouse.down(fromElViewport, forceEl);
        ...
        onTable(domEvents) {
          return {
            1: () => {
              return {
                name: 'Mouse Events',
                data: [...formatMouseEvents(domEvents.moveEvents.events), ...formatMouseEvents(domEvents.clickEvents)].filter(Boolean),
              };
            },
          };
        },
    ...
    mousemove(subject, positionOrX, y, options = {}) {
      return mouseAction('mousemove', {
        y,
        subject,
        userOptions: options,
        positionOrX,
        onReady(fromElViewport, forceEl) {
          const clickEvents = mouse.move(fromElViewport, forceEl);
    ...
    mouseup(subject, positionOrX, y, options = {}) {
      return mouseAction('mouseup', {
        ...
        positionOrX,
        onReady(fromElViewport, forceEl) {
          const clickEvents = mouse.up(fromElViewport, forceEl);
