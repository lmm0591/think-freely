...
import { ElWindowPostion, ElViewportPostion, ElementPositioning } from '../dom/coordinates';
...
const ensureIsDescendent = ($el1, $el2, name: string, onFail) => {
  if (!$dom.isDescendent($el1, $el2)) {
    ...
    if ($dom.isWithinShadowRoot($el1.get(0)) && $el1.get(0).getRootNode() === $el2?.get(0).shadowRoot) {
      return;
    }
