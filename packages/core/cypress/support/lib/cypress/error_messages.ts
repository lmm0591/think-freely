...
export default {
  ...
  check_uncheck: {
    ...
    element_missing_value_attribute: {
      message: `{{node}} cannot be checked/unchecked because it has no \`value\` attribute and ${cmd(
        '{{cmd}}',
      )} was supplied a value argument`,
      docsUrl: 'https://on.cypress.io/{{cmd}}',
    },
  },
  ...
  contains: {
    ...
    length_option: {
      message: `${cmd(
        'contains',
      )} only ever returns one element, so you cannot assert on a \`length\` greater than one.`,
      docsUrl: 'https://on.cypress.io/contains',
    },
    regex_conflict: {
  ...
  cookies: {
    ...
    removed(obj) {
      return {
        message: `${cmd(obj.cmd)} was removed in Cypress version 12.0.0. Please update to use ${cmd(
          'session',
        )} instead.`,
  ...
  miscellaneous: {
    ...
    invalid_new_command: {
      message:
        '`Cypress.Commands.add()` is used to create new commands, but `{{name}}` is an existing Cypress command.\n\nPlease use `Cypress.Commands.overwrite()` if you would like to overwrite an existing command.\n',
      docsUrl: 'https://on.cypress.io/custom-commands',
    },
    reserved_command: {
      message:
        '`Cypress.Commands.add()` cannot create a new command named `{{name}}` because that name is reserved internally by Cypress.',
      docsUrl: 'https://on.cypress.io/custom-commands',
    },
    invalid_new_query: {
      message:
        '`Cypress.Commands.addQuery()` is used to create new queries, but `{{name}}` is an existing Cypress command or query, or is reserved internally by Cypress.\n\n If you want to override an existing command or query, use `Cypress.Commands.overrideQuery()` instead.',
      docsUrl: 'https://on.cypress.io/api/custom-queries',
    },
    reserved_command_query: {
      message:
        '`Cypress.Commands.addQuery()` cannot create a new query named `{{name}}` because that name is reserved internally by Cypress.',
      docsUrl: 'https://on.cypress.io/api/custom-queries',
    },
    invalid_overwrite: {
  ...
  mount: {
    ...
    vue_yielded_value: {
      message:
        'As of Cypress 11, mount now yields an object with VueWrapper and the component as properties. Destructure using `{ wrapper, component }` to access the VueWrapper and component.',
      docsUrl: 'https://on.cypress.io/migration-11-0-0-component-testing-updates',
    },
    alias(alias: string) {
      return {
  ...
  net_stubbing: {
    ...
    intercept: {
      extra_arguments: ({ argsLength, overload }) => {
        return cyStripIndent(
          `\
          The ${cmd('intercept', overload.join(', '))} signature accepts a maximum of ${
            overload.length
          } arguments, but ${argsLength} arguments were passed.

          Please refer to the docs for all accepted signatures for ${cmd('intercept')}.`,
          10,
        );
      },
      ...
      invalid_middleware_handler: ({ handler }) => {
        return cyStripIndent(
          ...
          ${cmd(
            'intercept',
          )}'s \`handler\` argument must be an HttpController function when \`middleware\` is set to \`true\`.
      ...
      no_duplicate_url: `When invoking ${cmd(
        'intercept',
      )} with a \`RouteMatcher\` as the second parameter, \`url\` can only be specified as the first parameter.`,
      handler_required: `When invoking ${cmd(
        'intercept',
      )} with a \`RouteMatcher\` as the second parameter, a handler (function or \`StaticResponse\`) must be specified as the third parameter. If you intended to stub out a response body by passing an object as the 2nd parameter, pass an object with a \`body\` property containing the desired response body instead.`,
    },
    request_handling: {
      ...
      completion_called_after_resolved: ({ cmd }) => {
        return cyStripIndent(
          `\
          \`req.${cmd}()\` was called after the request handler finished executing, but \`req.${cmd}()\` can not be called after the request has already completed.`,
          10,
        );
      },
  ...
  origin: {
    ...
    invalid_url_argument: {
      message: `${cmd(
        'origin',
      )} requires the first argument to be either a url (\`https://www.example.com/path\`) or a domain name (\`example.com\`). Query parameters are not allowed. You passed: \`{{arg}}\``,
    },
    invalid_url_argument_same_origin({ originUrl, topOrigin, policy }) {
      return stripIndent`\
      ${cmd('origin')} requires the first argument to be a different ${
        policy === 'same-origin' ? 'origin' : 'domain'
      } than top. You passed \`${originUrl}\` to the origin command, while top is at \`${topOrigin}\`.
    ...
    ran_origin_fn_reference_error: {
      message: stripIndent`
        ...
        Note: Using \`require()\` or \`import()\` within ${cmd(
          'origin',
        )} from a \`node_modules\` plugin is not currently supported.`,
    },
    ...
    failed_to_serialize_object: {
      ...
      ${cmd(
        'origin',
      )} could not serialize the subject due to one of its properties not being supported by the structured clone algorithm.
    ...
    failed_to_serialize_function: {
      ...
      ${cmd(
        'origin',
      )} could not serialize the subject due to functions not being supported by the structured clone algorithm.`,
    },
    failed_to_serialize_symbol: {
      message: stripIndent`\
      ${cmd(
        'origin',
      )} could not serialize the subject due to symbols not being supported by the structured clone algorithm.`,
    },
    failed_to_serialize_or_map_thrown_value: {
      message: stripIndent`\
      ${cmd(
        'origin',
      )} could not serialize the thrown value. Please make sure the value being thrown is supported by the structured clone algorithm.`,
    },
    failed_to_create_spec_bridge: {
      message: stripIndent`\
      ${cmd(
        'origin',
      )} failed to create a spec bridge to communicate with the specified origin. This can happen when you attempt to create a spec bridge to an insecure (http) frame from a secure (https) frame.
    ...
    unsupported: {
      origin: {
        message: `${cmd('origin')} use is not currently supported in the ${cmd(
          'origin',
        )} callback, but is planned for a future release. Please 👍 the following issue and leave a comment with your use-case:`,
        docsUrl: 'https://on.cypress.io/github-issue/20718',
      },
      intercept: {
        message: `${cmd('intercept')} use is not supported in the ${cmd(
          'origin',
        )} callback. Consider using it outside of the callback instead. Otherwise, please 👍 the following issue and leave a comment with your use-case:`,
        docsUrl: 'https://on.cypress.io/github-issue/20720',
      },
      session: {
        message: `${cmd('session')} use is not supported in the ${cmd(
          'origin',
        )} callback. Consider using it outside of the callback instead. Otherwise, please 👍 the following issue and leave a comment with your use-case:`,
        docsUrl: 'https://on.cypress.io/github-issue/20721',
      },
      Cypress_session: {
        message: `\`Cypress.session.*\` methods are not supported in the ${cmd(
          'origin',
  ...
  select: {
    invalid_argument: {
      message: `${cmd(
        'select',
      )} must be passed a string, number, or array as its 1st argument. You passed: \`{{value}}\`.`,
      docsUrl: 'https://on.cypress.io/select',
    },
    invalid_array_argument: {
      message: `${cmd(
        'select',
      )} must be passed an array containing only strings and/or numbers. You passed: \`{{value}}\`.`,
      docsUrl: 'https://on.cypress.io/select',
    },
    invalid_element: {
    ...
    invalid_number: {
      message: `${cmd(
        'select',
      )} was called with an invalid index: \`{{index}}\`. Index must be a non-negative integer.`,
      docsUrl: 'https://on.cypress.io/select',
    },
    multiple_elements: {
    ...
    no_matches: {
      message: `${cmd(
        'select',
      )} failed because it could not find a single \`<option>\` with value, index, or text matching: \`{{value}}\``,
      docsUrl: 'https://on.cypress.io/select',
    },
    option_disabled: {
      message: `${cmd(
  ...
  selectFile: {
    ...
    invalid_array_file_reference: {
      message: `${cmd(
        'selectFile',
      )} must be passed an array of Buffers or objects with non-null \`contents\`. At files[{{index}}] you passed: \`{{file}}\`.`,
    },
    invalid_single_file_reference: {
      message: `${cmd(
        'selectFile',
      )} must be passed a Buffer or an object with a non-null \`contents\` property as its 1st argument. You passed: \`{{file}}\`.`,
    },
    multiple_elements: {
    ...
    not_file_input: {
      message: `${cmd(
        'selectFile',
      )} can only be called on an \`<input type="file">\` or a \`<label for="fileInput">\` pointing to or containing a file input, but received the element:
    ...
    invalid_alias: {
      message: `${cmd(
        'selectFile',
      )} can only attach strings, Buffers or objects, while your alias \`{{alias}}\` resolved to: \`{{subject}}\`.`,
    },
  },
  ...
  selector_playground: {
    ...
    defaults_invalid_priority: {
      message:
        '`Cypress.SelectorPlayground.defaults()` called with invalid `selectorPriority` property. It must be one of: `data-*`, `id`, `class`, `tag`, `attributes`, `nth-child`. You passed: `{{arg}}`. Consider using the `onElement` property if a specific selector is desired.',
      docsUrl: 'https://on.cypress.io/selector-playground-api',
    },
  ...
  sessions: {
    ...
    session: {
      duplicateId({ id, hasUniqSetupDefinition, hasUniqValidateDefinition, hasUniqPersistence }) {
        ...
        return {
          message: stripIndent`
           This session already exists. You may not create a new session with a previously used identifier. If you want to create a new session with a different ${differences.join(
             ' and ',
           )}, please call ${cmd('session')} with a unique identifier other than **${id}**.`,
          docsUrl: 'https://on.cypress.io/session',
        };
      },
      wrongArgId: {
        ...
        ${cmd(
          'session',
        )} was passed an invalid argument. The first argument \`id\` must be an string or serializable object.
        `,
        docsUrl: 'https://on.cypress.io/session',
      },
      wrongArgSetup: {
      ...
      wrongArgOptionInvalid: {
        ...
        ${cmd(
          'session',
        )} was passed an invalid option value. **{{key}}** must be of type **{{expected}}** but was **{{actual}}**.
        `,
        docsUrl: 'https://on.cypress.io/session',
      },
      not_found: {
  ...
  should: {
    ...
    command_inside_should(obj) {
      return stripIndent`\
        ${cmd('should')} failed because you invoked a command inside the callback. ${cmd(
        'should',
      )} retries the inner function, which would result in commands being added to the queue multiple times. Use ${cmd(
        'then',
      )} instead of ${cmd('should')}, or move any commands outside the callback function.
  ...
  subject: {
    ...
    detached_during_actionability(obj) {
      return {
        message: stripIndent`\
          ${cmd(
            obj.name,
          )} failed because the page updated while this command was executing. Cypress tried to locate elements based on this query:
    ...
    detached_after_command(obj) {
      return {
        message: stripIndent`\
          ${cmd(
            obj.name,
          )} failed because the page updated as a result of this command, but you tried to continue the command chain. The subject is no longer attached to the DOM, and Cypress cannot requery the page after commands such as ${cmd(
          obj.name,
        )}.
    ...
    state_subject_deprecated: {
      message: `${cmd(
        'state',
        "'subject'",
      )} has been deprecated and will be removed in a future release. Consider migrating to ${cmd('subject')} instead.`,
    },
    state_withinsubject_deprecated: {
      message: `${cmd(
        'state',
        "'withinSubject'",
      )} has been deprecated and will be removed in a future release. You should read ${cmd(
        'state',
        "'withinSubjectChain'",
      )} once at the top of your command / query, and resolve it into a value with ${cmd(
        'getSubjectFromChain',
        'withinSubjectChain',
      )} as needed.`,
    },
  ...
  type: {
    ...
    invalid_datetime: {
      message: `Typing into a datetime input with ${cmd(
        'type',
      )} requires a valid datetime with the format \`YYYY-MM-DDThh:mm\` or \`YYYY-MM-DDThh:mm:ss\`, for example \`2017-06-01T08:30\`. You passed: \`{{chars}}\``,
      docsUrl: 'https://on.cypress.io/type',
    },
    invalid_month: {
  ...
  visit: {
    ...
    invalid_cross_origin_on_load(args) {
      return {
        message: stripIndent`${cmd(
          'visit',
        )} was called to visit a cross origin site with an \`onLoad\` callback. \`onLoad\` callbacks can only be used with same origin sites.
    ...
    invalid_cross_origin_on_before_load(args) {
      return {
        message: stripIndent`${cmd(
          'visit',
        )} was called to visit a cross origin site with an \`onBeforeLoad\` callback. \`onBeforeLoad\` callbacks can only be used with same origin sites.
  ...
  wait: {
    ...
    invalid_arguments_function: {
      message: `${cmd(
        'wait',
      )} was passed invalid arguments. You cannot pass a function. If you would like to wait on the result of a ${cmd(
        'wait',
      )}, use ${cmd('then')}.`,
      docsUrl: 'https://on.cypress.io/wait',
    },
  ...
  within: {
    ...
    multiple_elements(args) {
      return {
        ...
        ${cmd(
          'within',
        )} can only be called on a single element. Your subject contained {{num}} elements. Narrow down your subject to a single element (using \`.first()\`, for example) before calling \`.within()\`.
