...
import { HTMLElementCanSetSelectionRange, HTMLTextLikeElement } from './types';
...
export const isButtonLike = (el: HTMLElement) => {
  ...
  return _.some([type('button'), type('image'), type('reset'), type('submit'), type('checkbox'), type('radio')]);
};
...
export const getFirstFixedOrStickyPositionParent = ($el): JQuery<any> | null => {
  ...
  return findParent($el.get(0), (node) => {
    const wrapped = $jquery.wrap(node);
