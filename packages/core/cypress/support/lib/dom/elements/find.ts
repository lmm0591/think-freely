...
export const findParent = (el, condition) => {
  const collectParent = (node) => {
    ...
    if (parentMatchingCondition) {
      return parentMatchingCondition;
    }
...
export const getContainsSelector = (
  text,
  filter = '',
  options: {
    matchCase?: boolean;
  ...
  const expr = _.isRegExp(text) ? 'cy-contains-regex' : options.matchCase ? 'cy-contains' : 'cy-contains-insensitive';
